...
* **The Deep Dive:** Stop tokens are checked on IDs; stop strings require incremental byte/text matching because delimiters can span tokens and appear inside escaped content. Decide whether to include them and how to handle streaming rollback. Logit bias changes selected token logits but a visible string has multiple tokenizations, and extreme bias can create degenerate alternatives. Availability is uneven across frontier APIs: Anthropic's API does not expose `logit_bias`, and OpenAI's reasoning models do not support it, so the modern lever there is native structured outputs and grammar-constrained decoding; logit bias remains a real tool in open-weight serving stacks. Temperature/top-p affect syntax and semantic diversity; temperature zero is still not guaranteed deterministic across serving implementations.
