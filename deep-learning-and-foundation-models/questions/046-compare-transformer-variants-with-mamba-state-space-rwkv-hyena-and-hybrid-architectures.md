...
Compressed-state models have $O(1)$ state per layer at decode and near-linear long training, but must encode history into finite dimensions and may struggle with arbitrary exact recall, copying, or in-context binding. Attention has growing memory but directly revisits stored tokens. Hybrids interleave layers or add local/global attention.
